 ...
 * 2009 apr 15 Petr Žitný (petr@zitny.net)
...
void read_arguments(int argc, char **argv) {
	...
	char *mempos1=0,*endptr;
  	...
  	o1=0;
	int i=1;
	while ((c = getopt (argc, argv, "cjakwosiurthp:zyx")) != -1)
	{
	i++;
         switch (c)
           {
	   case 'c':
	     printf("Nazdar vole    %s       ",argv[i]);
	     break;
           case 'a':
  	     showall=1;
	     shownone=1;
	...
	if ( (pinfo!=0) | (shownone==0) ) {
		printf("Wireless Weather Station Reader v0.6\n");
		printf("(C) 2009 Petr Žitný\n\n");
		printf("options\n");
...
printf ("Humidity indoor\t%d %%\n", buf5.hindoor);
printf ("Humidity outdoor\t%d %%\n", buf5.houtdoor);
if ((buf2[243] & 128) > 0) {
	printf ("Temperature indoor\t-%d.%d C\n", buf5.tindoor / 10, abs(buf5.tindoor % 10));
} else {
	printf ("Temperature indoor\t%d.%d C\n", buf5.tindoor / 10, abs(buf5.tindoor % 10));
};
if ((buf2[246] & 128) > 0) {
	printf ("Temperature outdoor\t-%d.%d C\n", buf5.toutdoor / 10, abs(buf5.toutdoor % 10));
} else {
	printf ("Temperature outdoor\t%d.%d C\n", buf5.toutdoor / 10, abs(buf5.toutdoor % 10));
};
